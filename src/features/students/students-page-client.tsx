...
function normalizeTab(v: string | null): EStudentsTab {
  return Number(v) == EStudentsTab.GROUPS ? EStudentsTab.GROUPS : EStudentsTab.STUDENTS;
}

export default function StudentsPageClient() {
  const { get, getOr, navigateWith } = useMergedQuery();

  const tabFromUrl = normalizeTab(get(TAB));
  const qFromUrl = getOr(Q, '');

  const [tab, setTab] = useState<EStudentsTab>(tabFromUrl);
